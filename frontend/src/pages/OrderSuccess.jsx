...
export default function OrderSuccess() {
  ...
  const awaitingKhqr = order.paymentMethod === 'qr' && order.status === 'pending' && !paidNow;

  return (
    <div className="px-4 py-12 flex flex-col items-center text-center max-w-sm mx-auto">
      {!awaitingKhqr && (
      ...
      <h1 className="text-lg font-semibold text-gray-900 mb-1">
        {awaitingKhqr ? 'Scan to complete payment' : 'Order placed!'}
      </h1>
      ...
      {awaitingKhqr && (
        <div className="w-full mt-5">
          <KHQRPayment orderId={order.id} onPaid={() => setPaidNow(true)} />
        </div>
      )}
