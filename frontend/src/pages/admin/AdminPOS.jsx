...
export default function AdminPOS() {
  ...
  // KHQR (Bakong) — generated on demand when the cashier picks "QR Code" in
  // the checkout modal. Stateless on the backend (no order exists yet), so
  // this is just local UI state: the QR image/md5 to poll, and the poll
  // result. Once auto-detected as paid, we call the normal handleCheckout()
  // below exactly as if the cashier had clicked "Confirm Payment" — the
  // order is only ever created there, with server-computed totals.
  const khqrEnabled = settings?.khqrEnabled === true;
  const [khqrData, setKhqrData] = useState(null); // { qrImage, md5, expiresAt }
  const [khqrStatus, setKhqrStatus] = useState('idle'); // idle | loading | pending | paid | expired | error | checkout_failed
  const [khqrError, setKhqrError] = useState('');
  const [khqrSecondsLeft, setKhqrSecondsLeft] = useState(0);
  const khqrTimerRef = useRef(null);
  const [khqrCheckingNow, setKhqrCheckingNow] = useState(false);
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      showToast(err.message, 'error');
      // If this failure happened right after KHQR auto-detected a paid
      // transaction, don't leave the modal implying success (it was
      // already showing "Payment received" before this call). The money
      // has moved but no order was recorded — that's a state the cashier
      // needs to see clearly on the modal itself, not just a toast that
      // can be missed, since a fresh QR would ask the customer to pay a
      // second time for a sale that was never actually saved.
      if (paymentMethod === 'qr' && khqrStatus === 'paid') {
        clearInterval(khqrTimerRef.current);
        setKhqrError(err.message);
        setKhqrStatus('checkout_failed');
      }
    } finally {
  ...
  function resetKhqr() {
    ...
    setKhqrStatus('idle');
    setKhqrError('');
    setKhqrSecondsLeft(0);
  }

  async function generateKhqrCode() {
    if (grandTotal <= 0) return;
    resetKhqr();
    setKhqrStatus('loading');
    try {
      const data = await apiClient.post(
        '/admin/pos/khqr/generate',
        { amount: grandTotal, reference: `POS-${Date.now()}` },
        { token }
      );
      setKhqrData(data);
      setKhqrStatus('pending');
      const remaining = Math.floor((new Date(data.expiresAt) - Date.now()) / 1000);
      setKhqrSecondsLeft(Math.max(remaining, 0));
    } catch (err) {
      setKhqrError(err.message);
      setKhqrStatus('error');
    }
  ...
  // Generate (or regenerate) the KHQR code whenever the cashier lands on
  // the QR tab of an open checkout modal.
  useEffect(() => {
    if (showCheckoutModal && paymentMethod === 'qr' && khqrEnabled && khqrStatus === 'idle') {
      generateKhqrCode();
    }
    if (!showCheckoutModal || paymentMethod !== 'qr') {
      resetKhqr();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showCheckoutModal, paymentMethod, khqrEnabled]);

  // Payment status is checked only when the cashier presses "Check now" —
  // no background polling — since Bakong's daily request quota (sandbox
  // tokens often 100/day) is easy to burn through otherwise. On success,
  // auto-finalize the sale exactly like clicking "Confirm Payment".
  async function handleKhqrCheckNow() {
    if (!khqrData?.md5) return;
    setKhqrCheckingNow(true);
    setKhqrError('');
    try {
      const res = await apiClient.get(`/admin/pos/khqr/status/${khqrData.md5}`, { token });
      if (res.status === 'PAID') {
        setKhqrStatus('paid');
        handleCheckout();
      } else {
        setKhqrError("Not detected yet — if the customer has paid, wait a few seconds and check again.");
      }
    } catch (err) {
      setKhqrError(err.message || 'Could not verify payment status.');
    } finally {
      setKhqrCheckingNow(false);
    }
  }

  // Countdown for the current KHQR code's expiration.
  useEffect(() => {
    if (khqrStatus !== 'pending') return;
    khqrTimerRef.current = setInterval(() => {
      setKhqrSecondsLeft((prev) => {
        if (prev <= 1) {
          clearInterval(khqrTimerRef.current);
    ...
    return () => clearInterval(khqrTimerRef.current);
  }, [khqrStatus]);
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-[var(--radius-md)] max-w-sm w-full p-6 max-h-[92vh] overflow-y-auto">
            ...
            {paymentMethod === 'qr' && !khqrEnabled && (
              <div className="mb-4 flex flex-col items-center py-4 text-center">
                ...
                <p className="text-[11px] text-gray-400 mt-1">
                  KHQR isn't set up yet — enable it under Settings → Payment methods.
                </p>
              </div>
            )}
            ...
            {paymentMethod === 'qr' && khqrEnabled && (
              <div className="mb-4 flex flex-col items-center py-2">
                ...
                {khqrStatus === 'pending' && khqrData?.qrImage && (
                  <>
                    <img src={khqrData.qrImage} alt="KHQR payment code" className="w-44 h-44 object-contain mb-2" />
                    <p className="text-sm text-gray-600 mb-1">
                      Amount: <span className="font-semibold text-gray-900">{formatPrice(grandTotal)}</span>
                    </p>
                    <p className="font-mono text-xs text-gray-500 mb-1">
                      {String(Math.floor(khqrSecondsLeft / 60)).padStart(2, '0')}:{String(khqrSecondsLeft % 60).padStart(2, '0')}
                    </p>
                    <div className="flex items-center gap-1.5 text-[11px] text-primary-600">
                      <span className="h-1.5 w-1.5 rounded-full bg-primary-500 animate-pulse" />
                      Waiting for customer to pay…
                    </div>
                    <button
                      onClick={handleKhqrCheckNow}
                      disabled={khqrCheckingNow}
                      className="mt-2 px-3 py-1 rounded-[var(--radius-sm)] border border-primary-200 text-primary-700 text-[11px] font-medium disabled:opacity-60"
                    >
                      {khqrCheckingNow ? 'Checking…' : 'Check now'}
                    </button>
                    {khqrError && (
                      <p className="mt-1 text-[11px] text-gray-500 text-center max-w-[200px]">{khqrError}</p>
                    )}
                  </>
                )}

                {khqrStatus === 'paid' && (
                  <div className="w-40 h-40 flex flex-col items-center justify-center gap-2 text-success">
                ...
                {khqrStatus === 'expired' && (
                  <div className="flex flex-col items-center gap-2 py-4 text-center">
                    <p className="text-sm text-error">This code expired.</p>
                    <button
                      onClick={generateKhqrCode}
                      className="px-3 py-1.5 rounded-[var(--radius-sm)] bg-primary-600 text-white text-xs font-medium"
                    >
                      Generate new code
                    </button>
                  </div>
                ...
                {khqrStatus === 'error' && (
                  <div className="flex flex-col items-center gap-2 py-4 text-center">
                    <p className="text-sm text-error">{khqrError || 'Could not generate KHQR code.'}</p>
                    <button
                      onClick={generateKhqrCode}
                      className="px-3 py-1.5 rounded-[var(--radius-sm)] border border-gray-200 text-xs font-medium text-gray-700"
                    >
                      Try again
                    </button>
                  </div>
                ...
                {khqrStatus === 'checkout_failed' && (
                  <div className="flex flex-col items-center gap-2 py-4 text-center">
                    <p className="text-sm font-medium text-success">Payment received</p>
                    <p className="text-xs text-error">
                      But the sale could not be saved: {khqrError || 'Unknown error'}
                    </p>
                    <p className="text-[11px] text-gray-500">
                      The customer already paid — don't ask them to pay again. Fix the issue, then press Confirm Payment below to finish this sale.
                    </p>
                  </div>
                )}
              </div>
            ...
            {paymentMethod === 'qr' && khqrEnabled && khqrStatus === 'pending' && (
              <p className="text-[11px] text-gray-400 text-center -mt-2 mb-4">
                Detected automatically — or press Confirm Payment to mark it paid manually.
              </p>
            )}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={checkingOut || cashInsufficient || khqrStatus === 'paid'}
