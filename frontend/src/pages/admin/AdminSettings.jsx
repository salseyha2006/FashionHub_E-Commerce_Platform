...
export default function AdminSettings() {
  ...
  const [khqrEnabled, setKhqrEnabled] = useState(false);
  ...
  const [bakongTokenSet, setBakongTokenSet] = useState(false);
  ...
  useEffect(() => {
    apiClient.get('/admin/settings', { token })
      .then((s) => {
        ...
        setKhqrEnabled(s.khqrEnabled ?? false);
        setBakongAccountId(s.bakongAccountId || '');
        setBakongAccountNumber(s.bakongAccountNumber || '');
        setBakongAcquiringBank(s.bakongAcquiringBank || '');
        setBakongMerchantName(s.bakongMerchantName || '');
        setBakongMerchantCity(s.bakongMerchantCity || 'Phnom Penh');
        setKhqrCurrency(s.khqrCurrency || 'USD');
        setBakongTokenSet(!!s.bakongTokenSet);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        khqrEnabled,
        bakongAccountId: bakongAccountId || null,
        bakongAccountNumber: bakongAccountNumber || null,
        bakongAcquiringBank: bakongAcquiringBank || null,
        bakongMerchantName: bakongMerchantName || null,
        bakongMerchantCity: bakongMerchantCity || 'Phnom Penh',
        khqrCurrency,
      };
      // Only send bakongToken if the admin actually typed a new one —
      // leaving the field blank keeps whatever is already stored.
      if (bakongToken !== '') payload.bakongToken = bakongToken;
      const data = await apiClient.put('/admin/settings', payload, { token });
      setBakongTokenSet(!!data.bakongTokenSet);
      setBakongToken('');
  ...
  async function handleClearBakongToken() {
    try {
      const data = await apiClient.put('/admin/settings', { bakongToken: '' }, { token });
      setBakongTokenSet(!!data.bakongTokenSet);
      showToast('Bakong access token cleared.', 'success');
    } catch (err) {
      showToast(err.message, 'error');
    }
  }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <label className="flex items-center gap-2.5 text-sm text-gray-900">
              <input
                type="checkbox"
                checked={khqrEnabled}
                onChange={(e) => setKhqrEnabled(e.target.checked)}
                className="focus-ring h-4 w-4 accent-primary-500"
              />
              KHQR (Bakong) — auto-confirmed payment
            </label>

            {khqrEnabled && (
              <div className="flex flex-col gap-5 pl-6 border-l-2 border-gray-100">
                <p className="text-xs text-gray-500 -mt-1">
                  Requires a Bakong Merchant account. Customers scan a QR generated for the exact order
                  total; payment is detected automatically, no manual confirmation needed.
                </p>

                <div className="grid grid-cols-2 gap-4">
                  <Field label="Bakong Account ID">
                    <input
                  ...
                  <Field label="Currency">
                    <select value={khqrCurrency} onChange={(e) => setKhqrCurrency(e.target.value)} className={inputClass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Account number (behind the alias above)">
                    <input
                      value={bakongAccountNumber}
                      onChange={(e) => setBakongAccountNumber(e.target.value)}
                      placeholder="008407206"
                      className={inputClass}
                    />
                  </Field>
                  <Field label="Bank / Acquiring bank name">
                    <input
                ...
                <p className="text-[11px] text-gray-400 -mt-3">
                  Required for bank-issued account IDs (e.g. from ABA, Wing) — open your bank app's "Receive Money" /
                  KHQR screen to find your account number and bank name. Without these, scanning apps reject the code
                  as "Invalid Qr Merchant Data".
                </p>

                <div className="grid grid-cols-2 gap-4">
                  <Field label="Merchant name on QR (optional)">
                    <input
                ...
                <Field label={bakongTokenSet ? 'Bakong Open API access token (set — leave blank to keep it)' : 'Bakong Open API access token'}>
                  <div className="flex gap-2">
                    <input
                      type="password"
                      value={bakongToken}
                      onChange={(e) => setBakongToken(e.target.value)}
                      placeholder={bakongTokenSet ? '••••••••' : ''}
                      className={inputClass}
                    />
                    {bakongTokenSet && (
                      <button type="button" onClick={handleClearBakongToken} className="focus-ring shrink-0 px-3 text-xs font-medium text-error hover:underline">
                        Clear
                      </button>
                    )}
                  </div>
                  <p className="text-[11px] text-gray-400 mt-1">
                    Separate from your Bakong Account ID — this token is issued by NBC/Bakong for the Open API
                    used to auto-check payment status. Without it, KHQR codes still generate but won't
                    auto-confirm.
                  </p>
                </Field>
              </div>
