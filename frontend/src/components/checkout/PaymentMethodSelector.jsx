...
export default function PaymentMethodSelector({ value, onNext, onBack }) {
  ...
  const [method, setMethod] = useState(
    value || (codEnabled ? 'cod' : bankTransferEnabled ? 'bank_transfer' : 'qr')
  );

  // If the selected method becomes unavailable once settings load, switch to the first available one
  useEffect(() => {
    const available = { cod: codEnabled, bank_transfer: bankTransferEnabled, qr: khqrEnabled };
    if (!available[method]) {
      const fallback = Object.keys(available).find((m) => available[m]);
      if (fallback) setMethod(fallback);
    }
  }, [codEnabled, bankTransferEnabled, khqrEnabled, method]);
    ...
    <div className="flex flex-col gap-4">
      ...
      {khqrEnabled && (
        <PaymentOption
          ...
          label="KHQR (Bakong)"
          description="Scan with any bank app — confirmed instantly"
        />
      )}
      ...
      {khqrEnabled && method === 'qr' && (
        <div className="border border-gray-200 rounded-[var(--radius-lg)] p-4 text-sm text-gray-600 bg-gray-50">
          You'll get a KHQR code to scan on the next step — payment is confirmed automatically once you pay.
        </div>
      )}
