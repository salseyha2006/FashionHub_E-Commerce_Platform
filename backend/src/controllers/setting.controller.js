...
const AUDIT_REDACTED_FIELDS = new Set(['smtpPasswordEncrypted', 'bankAccountNumber', 'bakongTokenEncrypted']);
...
function toSettingDTO(s) {
  return {
    ...
    khqrEnabled: s.khqrEnabled,
    bakongAccountId: s.bakongAccountId,
    bakongAccountNumber: s.bakongAccountNumber,
    bakongAcquiringBank: s.bakongAcquiringBank,
    bakongMerchantName: s.bakongMerchantName,
    bakongMerchantCity: s.bakongMerchantCity,
    khqrCurrency: s.khqrCurrency,
    bakongTokenSet: !!s.bakongTokenEncrypted, // never return the encrypted blob itself
...
async function updateSettings(req, res) {
  try {
    const {
      ...
      khqrEnabled, bakongAccountId, bakongAccountNumber, bakongAcquiringBank, bakongMerchantName, bakongMerchantCity, khqrCurrency, bakongToken,
    ...
    const errors = validateStoreSettingInput(
      {
        ...
        khqrEnabled, khqrCurrency, bakongAccountId,
      },
      { partial: true }
    );
    if (errors.length > 0) {
    ...
    const data = {
      ...
      ...(khqrEnabled !== undefined && { khqrEnabled }),
      ...(bakongAccountId !== undefined && { bakongAccountId: bakongAccountId || null }),
      ...(bakongAccountNumber !== undefined && { bakongAccountNumber: bakongAccountNumber || null }),
      ...(bakongAcquiringBank !== undefined && { bakongAcquiringBank: bakongAcquiringBank || null }),
      ...(bakongMerchantName !== undefined && { bakongMerchantName: bakongMerchantName || null }),
      ...(bakongMerchantCity !== undefined && { bakongMerchantCity: bakongMerchantCity || 'Phnom Penh' }),
      ...(khqrCurrency !== undefined && { khqrCurrency: khqrCurrency || 'USD' }),
    ...
    // bakongToken follows the same write-only pattern as smtpPassword above.
    if (bakongToken !== undefined) {
      data.bakongTokenEncrypted = bakongToken ? encrypt(bakongToken) : null;
    }
