...
function validateOrderInput({ shippingAddress, phone, paymentMethod }) {
  const errors = [];
  const validPaymentMethods = ['cod', 'bank_transfer', 'qr'];
  ...
  if (!paymentMethod || !validPaymentMethods.includes(paymentMethod)) {
    errors.push('paymentMethod must be one of: cod, bank_transfer, qr');
  }
...
function validateStoreSettingInput(
  ...
  if (khqrEnabled !== undefined && typeof khqrEnabled !== 'boolean') {
    errors.push('khqrEnabled must be a boolean');
  }

  if (khqrCurrency !== undefined && khqrCurrency !== null && !['KHR', 'USD'].includes(khqrCurrency)) {
    errors.push('khqrCurrency must be one of: KHR, USD');
  }

  if (bakongAccountId !== undefined && bakongAccountId !== null && bakongAccountId !== '') {
    if (typeof bakongAccountId !== 'string' || bakongAccountId.length > 32) {
      errors.push('bakongAccountId must be a string up to 32 characters');
    }
  }
