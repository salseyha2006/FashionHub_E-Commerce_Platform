...
function toOrderDetail(order) {
  return {
    ...
    paidAt: order.paidAt,
    // Only relevant while still pending — lets the frontend resume polling
    // an already-generated KHQR without calling /khqr/generate again.
    khqrExpiresAt: order.status === 'pending' ? order.khqrExpiresAt : null,
